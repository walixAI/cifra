...
// enlace. En desarrollo, el enlace no se manda de verdad si no hay AUTH_RESEND_KEY (lib/correo.ts
...
export default function PaginaVerificar() {
  const correoSimulado = process.env.NODE_ENV !== "production" && !process.env.AUTH_RESEND_KEY;
    ...
    <main className="mx-auto flex min-h-dvh max-w-sm flex-col items-center justify-center px-5 py-10 text-center">
      ...
      {correoSimulado && (
        <p className="mt-4 rounded-lg px-3 py-2 text-xs" style={{ background: "var(--chip)", color: "var(--muted)" }}>
          Estás en desarrollo sin <code>AUTH_RESEND_KEY</code> configurado — no se mandó ningún
